...
def init(ctx, company: bool, portfolio: bool, force: bool):
    ...
    # agent. Dropping an externally-executed agent (globexapp's `box-scout`)
...
def pull(
    ...
    * **Over SSH on the hub** (`--node acme`): the deployed shape — the
    ...
    **The worker-side contract** (the ADO ground-truth check itself is out of
    ...
    bead, query the external system of record — for the Acme lane, the ADO
