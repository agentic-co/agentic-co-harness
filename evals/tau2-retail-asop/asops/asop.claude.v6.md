...
- **G1 — Authenticate first.** At the start of the conversation, establish the user's identity by locating their user id from their email, or from name plus zip code. Do this even when the user has already stated a user id. Nothing else may proceed until it is done. Gate: deterministic (tool call: `find_user_id`).
...
- **G8 — Finish the request.** Once preconditions are met, carry the procedure to its final step and artifact — do not stop at authentication, a lookup, or confirmation alone.
...
1. **Authenticate the user.** Precondition: a tool lookup has returned this user's id, found from their email or from their name plus zip code. Do this even if the user has stated a user id — a stated id is not authentication. Gate: deterministic (tool call: `find_user_id`).
2. **Look up the answer, then give it.** Preconditions: the question concerns THIS authenticated user's own profile, orders, or products related to them; and a tool result containing the answer has been retrieved at any point in this conversation — a turn spent making that lookup satisfies this step even though the reply comes on a later turn. Prohibition: do not answer about another user; do not state detail no tool returned; do not offer opinions or recommendations. Gate: judged (the information given traces to some tool result in the conversation).
...
1. **Authenticate the user.** Precondition: a tool lookup has returned this user's id, found from their email or from their name plus zip code. Do this even if the user has stated a user id. Gate: deterministic (tool call: `find_user_id`).
2. **Obtain the order id and check its status.** Preconditions: the user has confirmed which order; a tool result shows that order's status is **pending**. An order that is processed, delivered, or cancelled cannot be cancelled here — check the status before acting rather than assuming it. Prohibition: if the status is not pending, deny and explain. Gate: deterministic (tool call: `get_order_details`).
...
4. **Confirm and cancel.** Precondition: before calling the cancellation tool, list the action details and obtain explicit user confirmation (yes) to proceed. Gate: human, then deterministic (tool call: `cancel_pending_order`). Artifact: order with status 'cancelled'.
...
1. **Authenticate the user.** Precondition: a tool lookup has returned this user's id, found from their email or from their name plus zip code. Do this even if the user has stated a user id. Gate: deterministic (tool call: `find_user_id`).
2. **Obtain the order id and check its status.** Preconditions: the user has confirmed which order; a tool result shows that order's status is **pending**. Prohibition: if the status is not pending, deny and explain. Gate: deterministic (tool call: `get_order_details`).
3. **Modify the shipping address**, if requested. Decide N/A FIRST: if no visible user turn asks to change the address, this step does not apply and the verdict is N/A, never a refusal — an address that was never mentioned is not a missing precondition. Only if a change was asked for, Precondition: the new address has been supplied by the user. Gate: human, then deterministic (tool call: `modify_pending_order_address`).
4. **Modify the payment method**, if requested. Decide N/A FIRST: if no visible user turn asks to change the payment method, this step does not apply and the verdict is N/A, never a refusal. Only if a change was asked for, Preconditions: exactly one new payment method, and it differs from the original; if it is a gift card, a tool result shows its balance covers the order total. Prohibition: refuse more than one method, or the same method as the original. After the change the order stays **pending**; the original method is refunded immediately if it is a gift card, otherwise within 5 to 7 business days. Gate: human, then deterministic (tool call: `modify_pending_order_payment`).
5. **Modify item options**, if requested. N/A when the user has not asked to change items — in that case this step does not apply and must be marked N/A rather than failed. Preconditions: **every** item the user wants changed has been collected into ONE list before the tool is called, and the user has been asked in any wording whether that list is complete and has answered — this tool can be called only once per order and afterwards the order can no longer be modified or cancelled. No particular phrasing is required. Each replacement is an available variant item of the **same product** with different options — item id, not product id; there is no change of product type (no shirt to shoe); the user has supplied a payment method for the price difference, and if it is a gift card a tool result shows its balance covers that difference. Prohibition: do not call the tool with a partial list; do not change product type. Gate: human, then deterministic (tool call: `modify_pending_order_items`). Artifact: order with status 'pending (items modified)'.
...
1. **Authenticate the user.** Precondition: a tool lookup has returned this user's id, found from their email or from their name plus zip code. Do this even if the user has stated a user id. Gate: deterministic (tool call: `find_user_id`).
2. **Obtain the order id and check its status.** Preconditions: the user has confirmed which order; a tool result shows that order's status is **delivered**. Prohibition: if the status is not delivered, deny and explain. Gate: deterministic (tool call: `get_order_details`).
...
5. **Confirm and request the return.** Precondition: before calling the return tool, list the action details and obtain explicit user confirmation (yes) to proceed. Gate: human, then deterministic (tool call: `return_delivered_order_items`). Artifact: order with status 'return requested'; the user receives an email explaining how to return the items.
...
1. **Authenticate the user.** Precondition: a tool lookup has returned this user's id, found from their email or from their name plus zip code. Do this even if the user has stated a user id. Gate: deterministic (tool call: `find_user_id`).
2. **Obtain the order id and check its status.** Preconditions: the user has confirmed which order; a tool result shows that order's status is **delivered**. Prohibition: if the status is not delivered, deny and explain. Gate: deterministic (tool call: `get_order_details`).
3. **Collect every item to be exchanged, in one list.** Precondition: the user has been asked, in any wording, whether the list of items is complete, and has answered — because the exchange tool can be called only **once per order**, so every item must be gathered before it is called. Asking "is that everything you want to exchange?" satisfies this; no particular phrasing is required. Each replacement is an available variant item of the **same product** with different options — item id, not product id; there is no change of product type (no shirt to shoe). Prohibition: do not call the tool with a partial list; do not change product type. Gate: human (user confirms the list is complete).
...
5. **Confirm and request the exchange.** Precondition: before calling the exchange tool, list the action details and obtain explicit user confirmation (yes) to proceed. Gate: human, then deterministic (tool call: `exchange_delivered_order_items`). Artifact: order with status 'exchange requested'; the user receives an email explaining how to return the items. No new order is placed.
