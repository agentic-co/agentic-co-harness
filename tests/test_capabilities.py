...
def test_requires_dedupes_and_preserves_order(tmp_path):
    ...
    task = beads.create(
        title="t",
        description="d",
        requires=["acme-code", "ado-write", "acme-code"],
    )
    assert task.requires == ["acme-code", "ado-write"]
...
def test_claim_succeeds_on_a_superset(tmp_path):
    ...
    got = beads.claim(
        task.id, WORKER, capabilities=["ado-write", "acme-code", "extra"]
    )
...
def test_claim_succeeds_on_an_exact_match(tmp_path):
    beads = _beads(tmp_path)
    task = beads.create(
        title="t", description="d", requires=["ado-write", "acme-code"]
    )
    assert (
        beads.claim(task.id, WORKER, capabilities=["acme-code", "ado-write"])
        is not None
    )
...
def test_claim_names_every_missing_capability(tmp_path):
    ...
    assert "ado-write" in message and "acme-code" in message
...
def test_ready_is_not_filtered_by_capability(tmp_path):
    """Visibility is NOT the gate. The hub must still see Acme work in its
...
def test_node_capabilities_load_from_config_yaml(tmp_path):
    config = _config_with(tmp_path, "capabilities: [ado-write, acme-code]\n")
    assert config.capabilities == ["ado-write", "acme-code"]
...
def test_local_child_is_not_remote(tmp_path):
    assert not ChildRef(name="globex", path=str(tmp_path)).is_remote


def test_legacy_child_row_without_the_new_tags_still_parses(tmp_path):
    ref = ChildRef.from_json(
        json.dumps({"name": "initech", "path": "/tmp/initech", "expected_interval": "1h"})
    )
...
def test_status_counts_a_remote_child_as_remote_not_verified(tmp_path):
    ...
    remote = [c for c in status["children"] if c["child"] == "acme"][0]
...
def test_tasks_create_requires_flag_is_repeatable(tmp_path):
    ...
    assert task.requires == ["ado-write", "acme-code"]
...
def test_pull_node_uses_the_registered_child_manifest_not_the_hubs(tmp_path):
    ...
    remote = json.loads(
        CliRunner()
        .invoke(main, ["--config", cfg, "pull", "--agent", WORKER, "--node", "acme"])
        .stdout
    )
...
def test_pull_refuses_an_unknown_node_rather_than_defaulting(tmp_path):
    ...
    result = CliRunner().invoke(
        main, ["--config", str(cfg), "pull", "--agent", WORKER, "--node", "acme"]
    )
    assert result.exit_code == 1
