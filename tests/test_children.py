...
# children (globex/vault-only, acme/ado-backed, personal/manual) were
...
ADO_BACKED = json.dumps({
    "name": "acme", "type": "ado-backed", "ado_org": "acme",
...
def test_all_six_production_registry_rows_parse(tmp_path):
    ...
    registry = tmp_path / "registry.jsonl"
    registry.write_text("\n".join([
        json.dumps({"name": "umbrella", "path": "/tmp/r", "expected_interval": "1h"}),
        json.dumps({"name": "feeds", "path": "/tmp/f", "expected_interval": "1d"}),
        json.dumps({"name": "initech", "path": "/tmp/m", "type": "beads", "expected_interval": "1h"}),
...
def test_stale_child_warns_when_parent_is_equally_late(tmp_path):
    """Umbrella 2026-08-31: child 6h31m past its deadline, parent 8h past its
