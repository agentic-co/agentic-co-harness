...
def _hub(tmp_path: Path, extra: str = "") -> Path:
    ...
    result = CliRunner().invoke(
        ...
        [
            "--config", str(cfg),
            "link-child", "acme", str(tmp_path / "acme"),
...
def test_capability_is_repeatable_and_the_flags_given_are_the_whole_set(tmp_path):
    ...
    """
    cfg = _hub(tmp_path)
    _run(cfg, "set-tags", "acme", "--capability", "ado-write", "--capability", "gpu")
    assert _registry(tmp_path).get("acme").capabilities == ["ado-write", "gpu"]

    _run(cfg, "set-tags", "acme", "--capability", "gpu")
    assert _registry(tmp_path).get("acme").capabilities == ["gpu"]
...
def test_an_invalid_capability_token_fails_loudly_and_writes_nothing(tmp_path):
    ...
    result = _run(cfg, "set-tags", "acme", "--capability", "ADO WRITE")
...
def test_clear_capabilities_empties_the_tag(tmp_path):
    ...
    result = _run(cfg, "set-tags", "acme", "--clear-capabilities")
    assert result.exit_code == 0, result.output
    assert _registry(tmp_path).get("acme").capabilities == []
...
def test_capability_and_clear_capabilities_together_is_refused(tmp_path):
    cfg = _hub(tmp_path)
    result = _run(
        cfg, "set-tags", "acme", "--capability", "gpu", "--clear-capabilities"
    )
    assert result.exit_code == 1
    assert _registry(tmp_path).get("acme").capabilities == []
...
def test_an_empty_host_string_is_refused(tmp_path):
    cfg = _hub(tmp_path)
    result = _run(cfg, "set-tags", "acme", "--host", "   ")
    assert result.exit_code == 1
    assert _registry(tmp_path).get("acme").host is None
...
def test_unknown_child_names_the_registry_and_exits_nonzero(tmp_path):
    ...
    assert [c.name for c in _registry(tmp_path).list()] == ["acme"]
...
def test_tags_written_by_the_cli_are_the_ones_the_claim_gate_reads(tmp_path):
    ...
    payload = json.loads(
        ...
        .invoke(
            main,
            ["--config", str(cfg), "pull", "--agent", WORKER, "--node", "acme"],
        )
        .stdout
    )
