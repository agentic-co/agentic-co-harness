...
def _portfolio(tmp_path: Path) -> tuple[Path, Path, Path]:
    """Parent + two children: globex (priority 1, healthy), initech (priority 3, healthy)."""
    parent = tmp_path / "parent"
    globex = tmp_path / "globex"
    initech = tmp_path / "initech"
    for root in (parent, globex, initech):
        _write_config(root)
        _fresh_heartbeat(root)
    _register_child(parent, "globex", globex, priority=1)
    _register_child(parent, "initech", initech, priority=3)
    return parent, globex, initech


def test_me_collects_across_children(tmp_path):
    parent, globex, initech = _portfolio(tmp_path)

    pb = Beads(str(parent / "tasks.jsonl"))
    t = pb.create("parent approval", "x")
    pb.update(t.id, status=TaskStatus.PENDING_APPROVAL)

    sb = Beads(str(globex / "tasks.jsonl"))
    st = sb.create("globex failure", "x")
    sb.update(st.id, status=TaskStatus.FAILED)

    mb = Beads(str(initech / "tasks.jsonl"))
    mt = mb.create("initech failure", "x")
    mb.update(mt.id, status=TaskStatus.FAILED)

    items = collect(str(parent / "config.yaml"))
    companies = {i.company for i in items}
    assert {"globex", "initech"} <= companies
    kinds = {(i.company, i.kind) for i in items}
    assert ("globex", "failed") in kinds
    assert ("initech", "failed") in kinds
...
def test_company_priority_orders_list(tmp_path):
    ...
    assert [i.company for i in failed] == ["globex", "initech"]
    assert failed[0].score > failed[1].score


def test_leverage_boosts_score(tmp_path):
    parent, globex, _ = _portfolio(tmp_path)
    b = Beads(str(globex / "tasks.jsonl"))
...
def test_registry_cycle_is_safe(tmp_path):
    parent, globex, _ = _portfolio(tmp_path)
    _register_child(globex, "parent-loop", parent, priority=2)
