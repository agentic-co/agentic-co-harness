...
def test_config_context_reaches_prompt(tmp_path):
    ...
    agent_config = AgentConfig(
        model="local-model", settings={"context": "UMBRELLA-CONTEXT-MARKER"}
    )
    ...
    assert "UMBRELLA-CONTEXT-MARKER" in all_content


def test_agent_subtasks_are_quarantined_for_approval(tmp_path):
    """REGRESSION (Umbrella subtask explosion, 2026-06-14): agent-proposed subtasks
