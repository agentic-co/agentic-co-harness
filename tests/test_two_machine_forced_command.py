...
PULL_OK = "agentco pull --agent acme-worker --node acme --max 3"
...
@pytest.mark.parametrize(
    "command",
    [
        "agentco pull --agent acme-worker",
        "agentco pull -a acme-worker --node acme",
        "agentco pull --agent acme-worker --reconcile",
        "agentco pull --agent acme-worker --ttl 900 --max 1",
        "agentco pull --agent acme-worker --reconcile-after 6.5",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "agentco report ac-b8700758 --attempt 1 --done --idempotency-key acme-worker:ac-b8700758:1",
...
def test_force_denied(hub):
    ...
    proc = run("agentco pull --agent acme-worker --force", hub)
    assert proc.returncode == DENIED
...
@pytest.mark.parametrize(
    "command",
    [
        "agentco pull --agent acme-worker --max abc",
        "agentco pull --agent acme-worker --ttl -5",
        "agentco pull --agent acme-worker --max",
        "agentco pull --agent --node acme",
        "agentco pull --node acme",
        "agentco pull --agent ../../etc/passwd",
        "agentco pull --agent /etc/passwd",
        "agentco pull --agent acme-worker --unknown-flag x",
        "agentco pull --agent acme-worker extra-positional",
    ],
)
